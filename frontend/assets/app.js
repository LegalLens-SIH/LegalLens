...
(function (global) {
  ...
  // The backend is always reached on port 8000, but on WHICHEVER hostname
  // this page itself was loaded from (localhost vs 127.0.0.1) - not
  // hardcoded to one, because browsers treat those as different sites for
  // SameSite cookie purposes. If this page is served from 127.0.0.1 while
  // every API call hardcodes "localhost", the login session cookie gets set
  // but then never sent back on the next request (SameSite=Lax blocks it
  // cross-site), which looks exactly like "login succeeds but doesn't let
  // you in". Matching the API host to the page's own host avoids that.
  var API_ORIGIN = "http://" + (global.location.hostname || "localhost") + ":8000";
  global.LL_API_ORIGIN = API_ORIGIN;
  ...
  // Compliance field values (backend/services/compliance_engine.py's
  // FieldResult.value, typed `Any`) are usually a plain string, but two
  // fields are structured objects: maximum_retail_price_mrp (on PASS -
  // _mrp_result) is {detected_value, normalized_value, currency}, and
  // consumer_care_details (whenever detected) is {name, address,
  // telephone_number, email_address}. Passing either straight into
  // escapeHtml() -> String(value) produces the literal text
  // "[object Object]" - this is the one place that must never happen, so
  // every renderer of a compliance field's `.value` should call this first.
  // Never hardcodes field NAMES or sample data - only the actual key shapes
  // the backend produces - and falls back to a readable "key: value" list
  // for any other structured value instead of silently reintroducing
  // "[object Object]" for a shape this doesn't already know.
  function formatComplianceValue(value) {
    if (value == null) return "";
    if (typeof value !== "object") return String(value);
    if (Array.isArray(value)) {
      return value.map(formatComplianceValue).filter(Boolean).join(", ");
    }
    // MRP shape (backend/services/compliance_engine.py::_mrp_result PASS branch).
    if ("normalized_value" in value || "detected_value" in value) {
      if (value.detected_value) return String(value.detected_value);
      var currencyPrefix = value.currency === "INR" ? "₹" : (value.currency ? value.currency + " " : "");
      return value.normalized_value != null ? currencyPrefix + value.normalized_value : "";
    }
    // Consumer care shape (backend/services/compliance_engine.py::normalize_ocr_result).
    if ("telephone_number" in value || "email_address" in value || "address" in value) {
      var parts = [];
      if (value.name) parts.push(value.name);
      if (value.telephone_number) parts.push("Phone: " + value.telephone_number);
      if (value.email_address) parts.push("Email: " + value.email_address);
      if (value.address) parts.push("Address: " + value.address);
      return parts.join(" • ");
    }
    // Unknown structured shape - readable key/value pairs, never "[object Object]".
    return Object.keys(value).map(function (key) {
      var v = value[key];
      if (v == null || v === "") return null;
      return key.replace(/_/g, " ") + ": " + (typeof v === "object" ? formatComplianceValue(v) : v);
    }).filter(Boolean).join(", ");
  }
  ...
  function logout() {
    ...
    if (global.fetch) {
      fetch(API_ORIGIN + "/api/auth/logout", { method: "POST", credentials: "include" }).catch(function () {});
    }
  }
  ...
  function normalizeManufacturerSidebar() {
    ...
    sidebar.querySelectorAll("a[href]").forEach(function (link) {
      ...
      // Root-cause fix: this used to grab the LAST <span> inside the link
      // and overwrite its text, assuming it was always a dedicated label
      // span (true on pages like manufacturer-dashboard.html, whose markup
      // is "<span class=material-symbols-outlined>icon</span><span
      // class=text-body-md>Label</span>"). On pages whose markup is just
      // "<span class=material-symbols-outlined>icon</span>Label" (a bare
      // trailing text node, e.g. self-check-report.html), the ICON span was
      // the only/last span - so its icon ligature name got overwritten with
      // the label text (e.g. "Operations"), which the Material Symbols
      // icon font then rendered as giant, malformed glyphs, right next to
      // the original untouched label text - exactly the reported "giant
      // decorative text" / "duplicated labels" bug. Fixed by explicitly
      // skipping the icon span and preferring a genuine label span if one
      // exists, else editing the existing trailing text node in place, and
      // only creating a new text node if neither is present - the icon
      // span itself is never touched.
      var labelSpan = null;
      var spans = link.querySelectorAll("span");
      for (var i = spans.length - 1; i >= 0; i -= 1) {
        if (!spans[i].classList.contains("material-symbols-outlined")) { labelSpan = spans[i]; break; }
      }
      if (labelSpan) {
        labelSpan.textContent = labels[path];
      } else {
        var labelTextNode = Array.prototype.slice.call(link.childNodes).reverse().find(function (node) {
          return node.nodeType === 3 && node.textContent.trim();
        });
        if (labelTextNode) labelTextNode.textContent = labels[path];
        else link.appendChild(document.createTextNode(labels[path]));
      }
  ...
  global.LL = {
    // utils
    qs: qs, escapeHtml: escapeHtml, formatComplianceValue: formatComplianceValue, fmtDateTime: fmtDateTime, fmtDate: fmtDate, toast: toast, comingSoon: comingSoon,
    // auth
